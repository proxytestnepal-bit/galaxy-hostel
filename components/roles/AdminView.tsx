...
const AdminView: React.FC<Props> = ({ activeTab, role }) => {
  ...
  const [selectedRoleForImp, setSelectedRoleForImp] = useState<Role | null>(
    null,
  );
  ...
  const [activeClassForSection, setActiveClassForSection] = useState<
    string | null
  >(null);
  ...
  const confirmApprovalOrEdit = () => {
    ...
    if (updates.role !== "teacher") {
      ...
      delete updates.teacherAssignments;
    }

    if (
      updates.allowedRoles &&
      updates.role &&
      !updates.allowedRoles.includes(updates.role)
    ) {
    ...
    if (reviewUser.status === "pending") {
      dispatch({
        type: "APPROVE_USER",
        payload: { id: reviewUser.id, updates },
      });
    } else {
      dispatch({
  ...
  const toggleSubjectInReview = (subjectName: string) => {
    ...
    const currentAssignments = reviewData.teacherAssignments || [];

    if (currentSubjects.includes(subjectName)) {
      setReviewData({
        ...reviewData,
        subjects: currentSubjects.filter((s) => s !== subjectName),
        teacherAssignments: currentAssignments.filter(
  ...
  const toggleAssignmentInReview = (subject: string, classId: string) => {
    const currentAssignments = [...(reviewData.teacherAssignments || [])];
    const index = currentAssignments.findIndex(
      (a) => a.subject === subject && a.classId === classId,
    );

    if (index >= 0) {
      currentAssignments.splice(index, 1);
    } else {
      currentAssignments.push({ subject, classId, sections: [] });
    }

    setReviewData({ ...reviewData, teacherAssignments: currentAssignments });
  };

  const toggleAssignmentSectionInReview = (
    subject: string,
    classId: string,
    section: string,
  ) => {
    ...
    if (index >= 0) {
      ...
      if (assignment.sections.includes(section)) {
        assignment.sections = assignment.sections.filter((s) => s !== section);
      } else {
        assignment.sections = [...assignment.sections, section];
      }
  ...
  const toggleAssignedClassInReview = (className: string) => {
    ...
    if (currentClasses.includes(className)) {
      ...
      setReviewData({
        ...reviewData,
        assignedClasses: [...currentClasses, className],
      });
    }
  };

  const toggleAssignedSectionInReview = (
    className: string,
    section: string,
  ) => {
  ...
  const toggleAllowedRoleInReview = (roleName: Role) => {
    ...
    if (currentAllowed.includes(roleName)) {
      setReviewData({
        ...reviewData,
        allowedRoles: currentAllowed.filter((r) => r !== roleName),
      });
    } else {
      setReviewData({
        ...reviewData,
        allowedRoles: [...currentAllowed, roleName],
      });
    }
  };
  ...
  const handleAddSubject = () => {
    if (newSubject.trim()) {
      dispatch({
        type: "ADD_SYSTEM_SUBJECT",
        payload: { name: newSubject.trim(), type: newSubjectType },
      });
      setNewSubject("");
    }
  };
  ...
  const handleAddSection = (className: string) => {
    if (newSection.trim()) {
      dispatch({
        ...
        payload: { className, section: newSection.trim() },
      });
      setNewSection("");
    }
  };
  ...
  const handleResetReceipt = () => {
    if (
      window.confirm(
        `Are you sure you want to reset the receipt counter to ${resetVal}? This is a critical action.`,
      )
    ) {
      ...
      <div className="space-y-8 max-w-4xl">
        <div className="bg-white p-8 rounded-2xl border border-galaxy-100 shadow-lg">
          <div className="flex items-center gap-3 mb-6">
            ...
            <div>
              <h3 className="text-xl font-bold text-galaxy-900">
                Issue New Notice
              </h3>
              <p className="text-sm text-gray-500">
                Communicate with students, staff, or everyone.
              </p>
            </div>
          </div>
          ...
          {state.notices.length === 0 ? (
            ...
            <div className="grid gap-4">
              {state.notices.slice(0, 5).map((notice) => (
                <div
                  key={notice.id}
                  className="bg-white p-5 rounded-2xl border border-gray-100 shadow-sm group hover:shadow-md transition-shadow"
                >
                  ...
                  <div className="flex justify-between items-center text-[10px] text-gray-400 border-t pt-3">
                    <span className="flex items-center gap-1">
                      <LogIn size={12} className="opacity-50" /> Posted By:{" "}
      ...
      return (
        <div className="space-y-6">
          ...
          <p className="text-gray-500 text-sm">
            Select a role to see available users. You can switch to their view
            without logging out.
          </p>
          <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-6">
            {roles.map((r) => (
              ...
              >
                <div
                  className={`w-16 h-16 rounded-full flex items-center justify-center mb-4 ${r.color}`}
                >
                  <r.icon size={32} />
                </div>
      ...
      <div className="space-y-6">
        ...
        <div className="bg-white p-4 rounded-xl shadow-sm border">
          <div className="relative">
            ...
            <input
              type="text"
              placeholder={`Search ${selectedRoleForImp}...`}
          ...
          {filteredUsers.map((user) => (
            ...
            >
              ...
              <button
                ...
                className="w-full bg-galaxy-900 text-white py-2 rounded-lg hover:bg-galaxy-700 mt-2 text-sm"
              >
                Act as {user.name.split(" ")[0]}
              </button>
            </div>
          ))}
        </div>
        ...
        <div className="bg-white p-6 rounded-xl border border-galaxy-200 shadow-sm">
          ...
          {getPendingUsers().length === 0 ? (
            <p className="text-gray-500 italic">
              No pending registration requests requiring your approval.
            </p>
          ) : (
            <div className="grid gap-4">
              {getPendingUsers().map((u) => (
                <div
                  key={u.id}
                  className="flex items-center justify-between border p-4 rounded-lg bg-yellow-50 border-yellow-100"
                >
                  <div>
                  ...
                  <div className="flex gap-2">
                    <button
                      ...
                      className="px-3 py-1 bg-white border border-red-200 text-red-600 rounded hover:bg-red-50 text-sm"
                    >
                      Reject
                    </button>
                    <button
                      onClick={() => openReviewModal(u)}
                      className="px-3 py-1 bg-galaxy-600 text-white rounded hover:bg-galaxy-700 text-sm flex items-center gap-1"
                    >
                      <Eye size={14} /> Review
                    </button>
                  </div>
              ...
              <div className="p-6">
                <div className="flex gap-2 mb-4">
                  ...
                  <select
                    ...
                    onChange={(e) =>
                      setNewSubjectType(e.target.value as SubjectType)
                    }
                  >
          ...
          <div className="fixed inset-0 bg-black/60 z-50 flex items-center justify-center p-4">
            <div className="bg-white rounded-xl shadow-xl max-w-lg w-full">
              <div className="p-4 border-b flex justify-between items-center bg-gray-50 rounded-t-xl">
                ...
                <button onClick={() => setShowClassManager(false)}>
                  <X size={18} />
                </button>
              </div>
              <div className="p-6">
                      ...
                      <div className="p-3 bg-white">
                        <div className="flex flex-wrap gap-2 mb-2">
                          {c.sections.map((sec) => (
                            <div
                              key={sec}
                              className="bg-blue-50 text-blue-800 text-xs px-2 py-1 rounded border border-blue-200 flex items-center gap-1"
                            >
                              {sec}
                              <button
                                onClick={() =>
                                  dispatch({
                                    type: "DELETE_CLASS_SECTION",
                                    payload: {
                                      className: c.name,
                                      section: sec,
                                    },
                        ...
                        <div className="flex gap-2 mt-2">
                          <input
                            ...
                            value={
                              activeClassForSection === c.name ? newSection : ""
                            }
                            onChange={(e) => {
                              setActiveClassForSection(c.name);
            ...
            <div className="bg-white rounded-xl shadow-xl w-full max-w-sm">
              <div className="bg-galaxy-900 text-white p-4 rounded-t-xl flex justify-between items-center">
                <h3 className="font-bold flex items-center gap-2">
                  <Key size={18} /> Reset User Password
                </h3>
                <button
                  onClick={() => {
                    setResetPassUserId(null);
              ...
              <div className="p-6">
                <p className="text-sm text-gray-600 mb-4">
                  You are resetting the password for this user. You cannot see
              ...
              <div className="p-6">
                <div className="flex items-start gap-4 mb-6">
                  ...
                  <div>
                    ...
                    <p className="text-sm text-gray-500">
                      {userToDelete.email} • {userToDelete.role.toUpperCase()}
                    </p>
                  </div>
                </div>
                  ...
                  >
                    <div>
                      ...
                      <span className="text-xs text-amber-700">
                        Removes access but keeps data. User is hidden from
                        active lists.
                      </span>
                    </div>
                    <Archive
                  ...
                  >
                    <div>
                      ...
                      <span className="text-xs text-red-700">
                        Completely removes user and data. Cannot be undone.
                      </span>
                    </div>
                    <Trash2
                      size={20}
          ...
          <div className="fixed inset-0 bg-black/60 z-50 flex items-center justify-center p-4">
            <div className="bg-white rounded-xl shadow-xl max-w-lg w-full overflow-hidden max-h-[90vh] overflow-y-auto">
              <div className="bg-galaxy-900 text-white p-4 flex justify-between items-center sticky top-0 z-10">
                ...
                <button onClick={() => setReviewUser(null)}>
                  <X size={20} />
                </button>
              </div>
              <div className="p-6 space-y-4">
                ...
                <div className="bg-gray-50 p-3 rounded-lg border">
                  ...
                  <div className="flex flex-wrap gap-2">
                    {(
                      ...
                      const isAllowed =
                        reviewData.allowedRoles?.includes(r) || isPrimary;
                      ...
                      return (
                        <button
                          ...
                          title={
                            isPrimary
                              ? "Primary role cannot be removed"
                  ...
                  <p className="text-[10px] text-gray-400 mt-1">
                    Users can switch between allowed roles in their dashboard.
                  </p>
                </div>
                ...
                <div className="border-t pt-4">
                  ...
                  {(reviewData.role === "teacher" ||
                    reviewData.allowedRoles?.includes("teacher")) && (
                    <div className="bg-purple-50 p-4 rounded-lg border border-purple-100 space-y-4">
                      ...
                      <div>
                        ...
                        <p className="text-[10px] text-gray-400 mb-3">
                          Select subjects and then configure which
                          classes/sections the teacher handles for each.
                        </p>

                        <div className="space-y-3">
                          {state.availableSubjects.map((subject) => {
                            const isSelected = reviewData.subjects?.includes(
                              subject.name,
                            );
                            return (
                              <div
                              ...
                              >
                                <div
                                  className={`p-2 flex items-center justify-between cursor-pointer transition-colors ${isSelected ? "bg-purple-100" : "hover:bg-gray-50"}`}
                                  onClick={() =>
                                    toggleSubjectInReview(subject.name)
                                  }
                                >
                                  <div className="flex items-center gap-2">
                                    <div
                                      className={`w-4 h-4 rounded border flex items-center justify-center ${isSelected ? "bg-purple-600 border-purple-600 text-white" : "border-gray-300"}`}
                                    >
                                  ...
                                  {isSelected && (
                                    <span className="text-[10px] text-purple-600 font-bold">
                                      {reviewData.teacherAssignments?.filter(
                                        ...
                                        return (
                                          ...
                                          >
                                            <label className="flex items-center gap-2 cursor-pointer mb-1">
                                              ...
                                              />
                                              <span
                                                className={`text-xs ${isClassAssigned ? "font-bold text-purple-700" : "text-gray-600"}`}
                                              >
                                                Class {cls.name}
                                              </span>
                                            </label>
                                                  ...
                                                  {cls.sections.map((sec) => {
                                                    ...
                                                    return (
                                                      <button
                                                        key={sec}
                                                        onClick={() =>
                                                          toggleAssignmentSectionInReview(
                                                            subject.name,
                                                            cls.name,
                                                            sec,
                                                          )
                                                        ...
                                                        className={`text-[9px] px-1.5 py-0.5 rounded border transition-all ${
                                                          isSecSelected
                                                            ...
                                                            : "bg-white text-gray-400 border-gray-200 hover:border-purple-200"
                                                        }`}
                                                      >
                                                        {sec}
                                                      </button>
                                            ...
                                            {isClassAssigned &&
                                              cls.sections.length === 0 && (
                                                <p className="text-[9px] text-gray-400 italic pl-5">
                                                  All Sections
                                                </p>
                                              )}
                                          </div>
        ...
        <div className="bg-white p-6 rounded-xl border border-gray-200 shadow-sm">
          <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-6 gap-4">
            ...
            <div className="flex gap-3 w-full md:w-auto items-center">
              ...
              >
                <BookOpen size={18} />
                <span className="hidden md:inline text-sm">Subjects</span>
              </button>
              ...
              >
                <Layout size={18} />
                <span className="hidden md:inline text-sm">Classes</span>
              </button>
                  ...
                  <tr key={u.id} className="hover:bg-gray-50 transition-colors">
                        ...
                        {u.allowedRoles &&
                          u.allowedRoles
                            ...
                            .map((r) => (
                              <span
                                key={r}
                                className="px-2 py-1 rounded text-xs border border-gray-300 text-gray-500 uppercase"
                              >
                                {r[0]}
                              </span>
                            ))}
                    ...
                    <td className="p-3">
                      <div className="flex gap-2">
                        <button
                          onClick={() => openReviewModal(u)}
                          className="text-galaxy-600 hover:bg-galaxy-100 p-2 rounded transition-colors"
                          title="Edit User"
                        >
                          <Edit2 size={16} />
                        </button>
  ...
  if (activeTab === "approvals") {
    const pendingFees = state.fees.filter(
      (f) => f.status === "pending_delete" || f.status === "pending_edit",
    );
    const pendingInvoices = state.invoices.filter(
      (i) => i.status === "pending_delete",
    );
          ...
          <div>
            ...
            <div className="space-y-4">
              {roleRequests.map((req) => (
                <div
                  key={req.id}
                  className="border border-purple-200 bg-purple-50 p-4 rounded-lg flex justify-between items-center shadow-sm"
                >
                  <div>
                    ...
                    <p className="text-sm text-purple-700">
                      ...
                      Requesting:{" "}
                      <span className="font-bold uppercase text-galaxy-600">
                        {req.requestedRole}
                      </span>
                    </p>
          ...
          roleRequests.length === 0 ? (
            ...
            <div className="space-y-4">
              {pendingInvoices.map((inv) => (
                <div
                  key={inv.id}
                  className="border border-orange-200 bg-orange-50 p-4 rounded-lg flex justify-between items-center"
                >
                  <div>
                    ...
                    <p className="text-sm">
                      {inv.title} - {inv.studentName} - Rs. {inv.amount}
                    </p>
                  </div>
                  <div className="flex gap-2">
                    <button
                      onClick={() =>
                        dispatch({
                          ...
                          payload: { id: inv.id, status: "unpaid" },
                        })
                      }
              ...
              {pendingFees.map((fee) => (
                ...
                >
                  <div>
                    ...
                    <p className="text-sm">
                      Receipt #{fee.receiptNumber} - {fee.studentName} - Rs.{" "}
                      {fee.amount}
                    </p>
                  </div>
                  <div className="flex gap-2">
  ...
  if (
    (activeTab === "system" || activeTab === "debug") &&
    role === "developer"
  ) {
    return (
      <div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          <div className="bg-white border border-red-200 p-6 rounded-xl shadow-sm">
            ...
            <p className="text-sm text-gray-600 mb-4">
              Current Next Receipt #: <strong>{state.receiptCounter}</strong>
              <br />
            </p>
            <div className="flex gap-2">
              <input
